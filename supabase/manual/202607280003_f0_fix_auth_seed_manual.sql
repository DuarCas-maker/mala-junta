...
-- 1) Asegura/actualiza usuarios Auth con contraseñas conocidas.
...
-- 2) Algunas versiones de GoTrue tienen columna confirmed_at; si existe, la rellenamos.
do $$
begin
  if exists (
    select 1
...
-- 3) Asegura identidades email. La columna id en GoTrue puede variar de tipo entre versiones;
-- usamos texto casteado dinámicamente para que funcione en self-hosted reciente.
do $$
declare
  v_id_type text;
begin
...
-- 4) Verificacion segura: no muestra hashes ni secretos.
select
  u.email,
