-- 025_f17_confirmar_captura_venta.sql
-- Fase 2: confirma una captura revisada, crea pedidos/pagos y descuenta inventario.

alter table public.capturas_venta add column if not exists confirmado_at timestamptz;
alter table public.capturas_venta add column if not exists confirmado_por uuid references public.perfiles(id) on delete restrict;

alter table public.captura_venta_grupos add column if not exists cuenta_id uuid references public.cuentas(id) on delete restrict;
alter table public.captura_venta_grupos add column if not exists pedido_id uuid references public.pedidos(id) on delete restrict;
alter table public.captura_venta_grupos add column if not exists confirmado_at timestamptz;
alter table public.captura_venta_grupos add column if not exists confirmado_por uuid references public.perfiles(id) on delete restrict;

create index if not exists captura_venta_grupos_pedido_idx on public.captura_venta_grupos(pedido_id) where pedido_id is not null;
create index if not exists captura_venta_grupos_cuenta_idx on public.captura_venta_grupos(cuenta_id) where cuenta_id is not null;
